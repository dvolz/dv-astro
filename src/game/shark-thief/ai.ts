...
export function moveEnemiesAI(): void {
  ...
  for (const e of gs.enemies) {
    ...
    tx = Math.max(0, Math.min(GRID - 1, tx));
    ty = Math.max(0, Math.min(GRID - 1, ty));
    const occupied     = gs.enemies.some(o => o !== e && o.x === tx && o.y === ty);
    const blockedCoral = !!gs.coral[ty]?.[tx];
    if (!occupied && !blockedCoral) { e.x = tx; e.y = ty; }
  }
  ...
  for (const be of gs.bigEnemies) {
    ...
    tx = Math.max(0, Math.min(GRID - 2, tx));
    ty = Math.max(0, Math.min(GRID - 2, ty));
    ...
    if (!blocked && !hitsEnemy && !hitsCoral) { be.x = tx; be.y = ty; }
  }
}
